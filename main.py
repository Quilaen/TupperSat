...
#        self.pressuren = "0x77" #setting location of the pressure module
    ...
    def extTemp(self):
        if self.ext:  # checking if we want to read the external
            self.ext.type2.seek(0)  # goes to the start of the ext temp file
            extM = self.ext.type2.readlines()  # reading the lines
    ...
    def intTemp(self):
        if self.int:  # checking if we want to read the internal temp
            self.int.type2.seek(0) #goes to the start of the int temp file
            intM = self.int.type2.readlines()  # reading the lines
